...
export function genPcCreateBlock(entity: naslTypes.Entity, parentElement: naslTypes.ViewElement, process: ProcessV2) {
  ...
  const nameGroup = {
    ...
    viewVariableEntity: firstLowerCase(entityName), // 新建的页面， 局部变量名根据实体名生成，不需要考虑唯一性
...
function genCreateFormTemplate(
  ...
  return `<ElCard style="width:100%;"
    slotHeader={
      <ElText text="发起流程" style="font-size: 18px; font-weight: bold;"></ElText>
    }
  >
    <ElForm ref="${nameGroup.viewElementMainView}" style="labelWidth: 100px;">
      <ElRow align="top">
        ...
        <ElCol span={24} style="text-align: center;">
          <ElButton type="primary" text="提交流程" style="margin-bottom:0px;"
            onClick={
