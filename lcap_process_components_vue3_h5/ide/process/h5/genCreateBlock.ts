...
function genCreateFormTemplate(entity: naslTypes.Entity, nameGroup: NameGroup, selectNameGroupMap: Map<string, NameGroup>) {
  ...
  return `${genCardTemplate(
    ...
    `<VanForm ref="${nameGroup.viewElementMainView}">
      ...
      <VanButton block={true} type="primary" text="提交流程"
        onClick={
 ...
 * @return {*}
 */
export function genH5CreateBlock(entity: naslTypes.Entity, parentElement: naslTypes.ViewElement, process: ProcessV2) {
  ...
  const nameGroup = {
    ...
    viewVariableEntity: likeComponent.getVariableUniqueName(firstLowerCase(entityName)),
  ...
  if (process) {
    const processV2LunchLogic = genProcessV2LaunchLogic(entityFullName, process, nameGroup);
